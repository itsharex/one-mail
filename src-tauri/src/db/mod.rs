    ...
    connection
        .execute(
            "UPDATE onemail_mail_accounts
             SET status='sync_error',last_error='上次同步意外中断，请重新刷新。',
                 updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now')
             WHERE status='syncing'",
            [],
        )
        .map(|_| ())
        .map_err(|error| format!("恢复中断的同步状态失败：{error}"))
}
...
#[cfg(test)]
mod tests {
    use super::{ensure_compatibility, recover_interrupted_syncs, SCHEMA_SQL};
    ...
    #[test]
    fn interrupted_sync_is_recovered_at_startup() {
        let connection = Connection::open_in_memory().expect("open in-memory database");
        connection.execute_batch(SCHEMA_SQL).expect("initialize schema");
        connection.execute(
            "INSERT INTO onemail_provider_presets (provider_key,display_name,auth_type)
             VALUES ('custom','Custom','manual')",
            [],
        ).expect("insert provider");
        connection.execute(
            "INSERT INTO onemail_mail_accounts
             (account_id,provider_key,email,normalized_email,account_label,auth_type,imap_host,imap_port,imap_security,status)
             VALUES (1,'custom','a@example.test','a@example.test','A','manual','localhost',993,'ssl_tls','syncing')",
            [],
        ).expect("insert interrupted account");
        recover_interrupted_syncs(&connection).expect("recover sync status");
        let (status, error): (String, String) = connection.query_row(
            "SELECT status,last_error FROM onemail_mail_accounts WHERE account_id=1",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        ).expect("load recovered account");
        assert_eq!(status, "sync_error");
