...
fn parse_message(value: &Value) -> Result<FetchedMessage, ApiError> {
    ...
    if value.get("@removed").is_some() {
        return Ok(FetchedMessage::deleted(stable_uid(id)));
    }
    ...
    let mut message = FetchedMessage {
        ...
        message_id: value
            ...
            .map(str::to_string),
        from_name: from
            .and_then(|from| from.get("name").and_then(Value::as_str).map(str::to_string)),
        from_email: from.and_then(|from| {
            from.get("address")
                .and_then(Value::as_str)
                .map(str::to_string)
        }),
    ...
    for (field, kind) in [
        ...
        if let Some(value) = value.get(field) {
            ...
            message
                .participants
                .push(crate::mail::body::ParticipantHeader { kind, addresses });
        }
    }
    Ok(message)
}
