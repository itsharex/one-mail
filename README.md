...
OneMail 使用以人为中心的「对话」布局：

1. **账号栏**：筛选往来账号、管理账号与同步。
2. **联系人列表**：按邮箱地址聚合往来，多人邮件按公开参与者成组，支持搜索和分页。
3. **对话区**：合并收到的邮件与本机已发送记录，按话题分隔、折叠引用与签名；正文进入视口后自动加载，底部直接回复或发起新话题，并明确显示发件账号、收件人和抄送。

正文中的网址和文字链接使用系统默认浏览器打开。历史参与者信息会在重新同步时回填；尚未同步到本机的往来不会出现在对话中。仅密送的发件记录仍从发件箱查看，密送地址不会进入公开会话参与者列表。
...
4. **查找对话**：按账号筛选往来，或在联系人列表中搜索名称和邮箱。
5. **阅读正文**：邮件进入视口后自动加载正文并更新本地已读状态；长正文可展开或收起。
6. **查看网页和图片**：点击正文链接在默认浏览器打开；HTML 模式下可按需加载本条邮件的远程图片。
7. **撰写和回复**：发起新对话，或在对话底部回复及创建新话题；写信窗口支持抄送、密送和本地附件。
8. **管理草稿和发件箱**：关闭有内容的写信窗口会保存草稿；在发件箱中继续编辑、重试发送或删除记录。
9. **可选 AI 助手**：在设置中验证自有 AI 服务后，可使用 AI 对话助手。
10. **备份数据**：在设置中导出 `.onemail` 备份，并可从本地备份恢复。
