...
export function AnimatedSegmentedTabs<TValue extends string = string>({
  ...
  highlightClassName,
  ...props
}: AnimatedSegmentedTabsProps<TValue>): React.JSX.Element {
  ...
  const moveHighlight = React.useCallback((nextValue: string, animate: boolean) => {
    const list = listRef.current
    const highlight = highlightRef.current
    const activeTrigger = triggerRefs.current.get(nextValue)

    if (!list || !highlight || !activeTrigger) return
